...
function initializeReportsModule() {
    // Reports are loaded when the tab is activated
}
...
function populateRevenueTable(revenueData) {
    ...
    if (revenueData.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="3" class="no-data">No revenue data found.</td></tr>';
        return;
    }
...
function populatePartUsageTable(partUsageData) {
    ...
    if (partUsageData.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="3" class="no-data">No part usage data found.</td></tr>';
        return;
    }
