// Enhanced Customer Dashboard with Map Integration and Notifications
...
function initializeMapIntegration() {
    // Initialize the map when the booking tab is active
    if (window.branchMapManager) {
        window.branchMapManager.initializeMap('branch-map-container');
        
        // Listen for branch selection
        window.addEventListener('branchSelected', (event) => {
            const selectedBranch = event.detail.branch;
            updateSelectedBranchInfo(selectedBranch);
        });
    }
}

function updateSelectedBranchInfo(branch) {
    const branchNameElement = document.getElementById('selected-branch-name');
    if (branchNameElement && branch) {
        branchNameElement.textContent = `${branch.name} - ${branch.address}`;
        branchNameElement.style.color = '#27ae60';
...
async function bookAppointment() {
    const userId = sessionStorage.getItem('userId');
    const selectedBranch = window.branchMapManager ? window.branchMapManager.getSelectedBranch() : null;
    
    if (!selectedBranch) {
        showError('Please select a branch location before booking.');
        return;
    }
        ...
        if (response.ok) {
            ...
            if (window.notificationManager) {
                window.notificationManager.notifyBooking(
                    document.getElementById('booking-service').options[document.getElementById('booking-service').selectedIndex].textContent.split(' - $')[0]
                );
            }
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (window.notificationManager) {
                ...
                window.notificationManager.notifyPayment(amount.replace('$', ''));
            }
...
// Service History Module (Enhanced)
...
// Customer Data Loading (Enhanced)
